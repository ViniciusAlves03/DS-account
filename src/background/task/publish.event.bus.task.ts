...
export class PublishEventBusTask implements IBackgroundTask {
    ...
    private _publishEvent(event: any): Promise<boolean> {
        if (event.event_name === UserDeleteEvent.NAME) {
            const user: User = new User().fromJSON(event.user)
            user.type = event.user.type
            const userDeleteEvent: UserDeleteEvent = new UserDeleteEvent(event.timestamp, user)
            return this._eventBus.publish(userDeleteEvent, event.__routing_key)
        }
        if (event.event_name === EmailWelcomeEvent.NAME) {
        ...
        if (event.event_name === EmailResetPasswordEvent.NAME) {
        ...
        if (event.event_name === EmailUpdatePasswordEvent.NAME) {
