...
@injectable()
export class HolderEntityMapper implements IEntityMapper<Holder, HolderEntity> {
    public transform(item: any): any {
        if (item instanceof Holder) return this.modelToModelEntity(item)
        return this.jsonToModel(item)
    }

    public modelToModelEntity(item: Holder): HolderEntity {
        const result: HolderEntity = new HolderEntity()
        ...
        if (item.dependents !== undefined && item.dependents.length) {
            result.dependents = item.dependents.map(patient => patient.id)
        }

        return result
    }

    public jsonToModel(json: any): Holder {
        const result: Holder = new Holder()
        ...
        if (json.dependents !== undefined && json.dependents.length) {
            result.dependents = json.dependents.map(item => {
                const dependent = new Dependent().fromJSON(item)
                dependent.id = item.id
                dependent.created_at = item.created_at
                dependent.updated_at = item.updated_at
